/**
 * Shared HTML template builders for storyboard pages.
 * Used by both the StoryboardComposerExecutor (canvas node) and
 * runStoryboardJob (GTM Kit integration pipeline).
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// Shared browser — one instance reused across all pages in a single job
// ─────────────────────────────────────────────────────────────────────────────
...
export function buildStoryboardPageHtml(opts: {
  scene: SceneObject
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sidebar {
    ...
    padding: 20px 18px; border-right: 1px solid #e5e7eb;
    display: flex; flex-direction: column; gap: 16px; overflow-y: auto;
  }
  .sidebar-label { font-size: 10px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: #9ca3af; margin-bottom: 4px; }
  .sidebar-content { font-size: 13px; line-height: 1.55; color: #1f2937; }
  .section-pill {
  ...
  .timecode { font-size: 11px; color: #6b7280; margin-bottom: 8px; }
  .animation-list { list-style: none; font-size: 12px; line-height: 1.7; color: #374151; padding-left: 0; }
  .frames-area {
    flex: 1; padding: 16px; display: grid; gap: 12px;
    ${frameGridCss(frameCount)}
    align-content: start;
  }
  .frame-cell { position: relative; background: #f3f4f6; border-radius: 6px; overflow: hidden; aspect-ratio: 16/9; }
  .frame-cell img { width: 100%; height: 100%; object-fit: cover; display: block; }
  .frame-label {
    position: absolute; top: 8px; right: 8px;
    background: ${primary}; color: #fff;
    font-size: 11px; font-weight: 700; padding: 2px 8px; border-radius: 4px; z-index: 1;
  }
  .footer { height: 32px; min-height: 32px; padding: 0 24px; display: flex; align-items: center; border-top: 1px solid #e5e7eb; color: #9ca3af; font-size: 11px; }
</style>
</head>
<body>
  <div class="header">
...
export function buildStoryboardCoverHtml(opts: {
  clientName: string
  verticalName: string
  version: string
  date: string
...
<style>
  ...
  .meta { font-family: ${bodyFont}; font-size: 14px; color: rgba(255,255,255,0.45); letter-spacing: 0.06em; text-transform: uppercase; }
  .accent-bar { width: 80px; height: 4px; background: ${secondary}; border-radius: 2px; margin: 28px auto; }
</style>
</head>
<body>
  ${logoHtml}
