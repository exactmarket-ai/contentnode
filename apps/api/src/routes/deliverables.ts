...
const MANAGER_PLUS = ['owner', 'admin', 'editor', 'manager', 'super_admin', 'org_admin']
...
export async function deliverablesRoutes(app: FastifyInstance) {
  ...
  app.get('/', { preHandler: requireRole(...MANAGER_PLUS) }, async (req, reply) => {
    ...
    req.log.info({ agencyId, clientCount: clients.length, memberCount: members.length }, '[deliverables] dropdown counts')
