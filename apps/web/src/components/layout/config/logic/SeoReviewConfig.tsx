...
export function SeoReviewConfig({
  config,
  onChange,
  nodeRunStatus,
}: {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">
                {mode === 'optimize'
                  ? 'SEO requirements are injected into the upstream generation prompt before content is generated'
        ...
        {/* Target keyword (SEO-specific) */}
        <FieldGroup label="Target keyword" description="Keyword or phrase that SEO evaluation and injection will focus on">
          <Input
            className="h-8 text-xs"
            placeholder="e.g. content marketing strategy"
            value={(config.target_keyword as string) ?? ''}
      ...
      {score !== undefined && !notApplicable && (
        <div className="space-y-2 border-t border-border pt-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">/ 100 SEO score</span>
          </div>
          {gatedNote && (
      ...
      <details className="border-t border-border pt-3">
        <summary className="cursor-pointer text-[11px] text-muted-foreground hover:text-foreground">
          SEO criteria reference (10 criteria)
        </summary>
        <ol className="mt-2 space-y-1 pl-4">
          {[
            ...
            'Content depth (word count relative to type)',
            'Keyword density (1–2%)',
