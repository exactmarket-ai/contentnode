...
export function SeoTab({ clientId, clientName }: { clientId: string; clientName: string }) {
  ...
  const loadData = useCallback(() => {
    setLoading(true)
    Promise.all([
      apiFetch(`/api/v1/seo/sessions?clientId=${clientId}`).then((r) => r.json()),
      apiFetch(`/api/v1/seo/briefs?clientId=${clientId}`).then((r) => r.json()),
    ])
  ...
  const handlePushToNewsroom = async (briefId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setBriefs((prev) => prev.map((b) => b.id === briefId ? { ...b, pushedToNewsroom: true } : b))
      }
    } finally {
      setPushingId(null)
    }
        ...
        <div className="flex items-center justify-between gap-2 mb-3">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              Guided seoPILOT sessions for {clientName}
            </p>
          </div>
          <button
