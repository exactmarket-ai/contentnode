...
// ── Content Library single item DOCX ─────────────────────────────────────────

export interface ContentLibraryDocxItem {
  topicTitle: string
  contentType: string
  assignedTo: string
  content: string
...
export async function downloadContentLibraryDocx(item: ContentLibraryDocxItem): Promise<void> {
  const date = new Date(item.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
  const children: Paragraph[] = [
    new Paragraph({
      children: [new TextRun({ text: item.clientName, size: 18, color: '666666' }), new TextRun({ text: '  ·  ', size: 18, color: 'CCCCCC' }), new TextRun({ text: item.assignedTo, size: 18, color: '666666' }), new TextRun({ text: '  ·  ', size: 18, color: 'CCCCCC' }), new TextRun({ text: date, size: 18, color: '666666' })],
      spacing: { after: 120 },
    }),
    new Paragraph({
      children: [new TextRun({ text: item.contentType, size: 16, color: '888888', italics: true })],
      spacing: { after: 240 },
    }),
    heading1(item.topicTitle),
    spacer(),
    ...markdownToDocxParagraphs(item.content),
    spacer(),
    new Paragraph({
      children: [new TextRun({ text: 'Generated by ContentNode', size: 16, color: 'AAAAAA', italics: true })],
      spacing: { before: 320 },
    }),
  ]
  ...
  const doc = new Document({
    styles: {
      paragraphStyles: [
        { id: 'Heading1', name: 'Heading 1', basedOn: 'Normal', next: 'Normal', run: { bold: true, size: 28, color: '1a1a2e' } },
        { id: 'Heading2', name: 'Heading 2', basedOn: 'Normal', next: 'Normal', run: { bold: true, size: 22, color: '534AB7' } },
      ],
    },
    sections: [{ children }],
  })
  ...
  const a = document.createElement('a')
  a.href = url
  const filename = `${slugify(item.topicTitle)}_${slugify(item.contentType)}_${new Date(item.createdAt).toISOString().slice(0, 10)}.docx`
  a.download = filename
  a.click()
  URL.revokeObjectURL(url)
}

export async function contentLibraryItemToBlob(item: ContentLibraryDocxItem): Promise<{ blob: Blob; filename: string }> {
  const date = new Date(item.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
  const children: Paragraph[] = [
    new Paragraph({
      children: [new TextRun({ text: item.clientName, size: 18, color: '666666' }), new TextRun({ text: '  ·  ', size: 18, color: 'CCCCCC' }), new TextRun({ text: item.assignedTo, size: 18, color: '666666' }), new TextRun({ text: '  ·  ', size: 18, color: 'CCCCCC' }), new TextRun({ text: date, size: 18, color: '666666' })],
  ...
  const blob = await Packer.toBlob(doc)
  const filename = `${slugify(item.contentType)}_${slugify(item.topicTitle)}.docx`
